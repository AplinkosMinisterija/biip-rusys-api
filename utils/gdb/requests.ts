...
export interface GdbPayload {
  layers: GdbLayer[];
  name: string;
  srid: number;
}

export interface GdbDataValidation {
  valid: boolean;
  message: string;
  placesCount: number;
...
export interface PlaceGdbFeature {
  placeId: number;
  placeCode: string;
  speciesId: number;
  speciesName: string;
...
export interface ObservationGdbFeature {
  formId: number;
  speciesId: number;
  speciesName: string;
  speciesNameLatin: string;
...
// The tools /gdb contract: one geometry type per layer. Mixed layers are
// split into per-type sub-layers (e.g. Radavietos_taskai, Radavietos_plotai).
function splitLayerByGeometryType(layer: GdbLayer): GdbLayer[] {
...
// Loads request data with the option set every GDB flow needs: places and
// observation forms with geometries, no PDF legend.
export async function getGdbRequestData(ctx: Context, id: number) {
...
export function buildGdbPayload(request: Request, requestData: any): GdbPayload {
  ...
  const observationLayers: GdbLayer = {
    name: 'Staigmenys',
    geojson: {
      type: 'FeatureCollection',
...
export function validateGdbData(requestData: any): GdbDataValidation {
